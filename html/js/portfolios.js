...
var currentPortfolio = allImages.galleries[0];
var html = "";

if (url.indexOf("#recent") >= 0) {
	currentPortfolio = allImages.galleries[0];
} else if (url.indexOf("#seasons") >= 0) {
	currentPortfolio = allImages.galleries[1];
} else if (url.indexOf("#seaandsky") >= 0) {
	currentPortfolio = allImages.galleries[2];
} else if (url.indexOf("#maine") >= 0) {
	currentPortfolio = allImages.galleries[3];
} else if (url.indexOf("#voyages") >= 0) {
	currentPortfolio = allImages.galleries[4];
}
else if (url.indexOf("#land") >= 0) {
	currentPortfolio = allImages.galleries[5];
}

var titleTemplate = "<h3>{{&title}}</h3>";
html = Mustache.to_html(titleTemplate, currentPortfolio);
$('div#titleTarget').html(html);

var bodyTemplate = "{{#images}}<li><a href = 'image.html{{&index}}'><img src = '{{&smallURL}}'></li>{{/images}}";
html = Mustache.to_html(bodyTemplate, currentPortfolio);
$('ul#portfolioTarget').html(html);
